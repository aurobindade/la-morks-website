...
const ANIM_DELIVERED = [
  { id: 1, init: "AF", name: "Angurbala Flims", desc: "Innovative animation studio specializing in 2D and 3D storytelling", imageUrl: "/assets/AngurbalaFlimsLogo.jpg" },
  { id: 2, init: "MP", name: "Motion Palette", desc: "Dynamic motion graphics for engaging digital experiences", imageUrl: "/assets/MotionPaletteLogo.png" },
  { id: 3, init: "BG", name: "Berhampore Girls' College", desc: "Educational institution focused on holistic development", imageUrl: "/assets/BerhamporeGirlsLogo.webp" },
  { id: 4, init: "PT", name: "Paschimer Janla", desc: "Subscribe and discover a new perspective, a new source of inspiration, with every video.", imageUrl: "/assets/YouTubeLOGO.png" },
];

const animServices = [
  { icon: "\u{1F3AD}", num: "01 \u2014 Animation", title: "2D / 3D Animation", desc: "Character design, cinematic sequences for film/TV/advertising, and photorealistic 3D product visualisation \u2014 before products even exist." },
  ...
  { icon: "\u{1F3A5}", num: "03 \u2014 VFX", title: "Visual Effects", desc: "Compositing, CGI integration, rotoscoping, professional colour grading, object removal, and Hollywood-standard green-screen keying." },
  { icon: "\u{1F916}", num: "04 \u2014 AI Creative", title: "AI-Powered Production", desc: "Multimodal Generative AI for concept art, character variations, and visual asset generation \u2014 accelerating timelines by up to 50%." },
  { icon: "\u{1F97D}", num: "05 \u2014 Immersive", title: "Interactive & Immersive", desc: "VR, AR, and interactive storytelling experiences \u2014 giving clients animated content that truly engages and immerses audiences." },
  { icon: "\u{1F3AC}", num: "06 \u2014 Production", title: "Real-Time Rendering", desc: "Unreal Engine-powered live animation and virtual production with instant client previews for agile creative iteration." },
];
...
export default function AnimationPage() {
  ...
  const getCurrentIndex3 = () => {
    ...
    const rootCenter = root.getBoundingClientRect().left + root.getBoundingClientRect().width / 2;
    let best = 0, bestDist = Infinity;
    items.forEach((el, i) => {
      if (!el) return;
      const d = Math.abs(el.getBoundingClientRect().left + el.getBoundingClientRect().width / 2 - rootCenter);
  ...
  const scrollToPhysicalIndex3 = (phys) => {
    ...
    catch { const root = portfolioRef3.current; if (root) root.scrollTo({ left: el.offsetLeft - root.offsetLeft - 8, behavior: 'smooth' }); }
  };
  ...
  const handleRecenterIfNeeded3 = () => {
    ...
    const target = phys < len ? phys + len : phys >= 2 * len ? phys - len : null;
    if (target == null) return;
    const el = portfolioItemRefs3.current[target];
    if (el) {
      try { el.scrollIntoView({ behavior: 'auto', inline: 'center', block: 'nearest' }); }
      catch { const root = portfolioRef3.current; if (root) root.scrollLeft = el.offsetLeft - root.offsetLeft - 8; }
    }
  };

  const handlePrev3 = () => {
    const cur = getCurrentIndex3();
    if (cur > 0) scrollToPhysicalIndex3(cur - 1);
  };

  const handleNext3 = () => {
    const cur = getCurrentIndex3();
    if (cur < displayAnimations.length - 1) scrollToPhysicalIndex3(cur + 1);
  };
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const root = portfolioRef3.current;
      const el = portfolioItemRefs3.current[len];
      if (root && el) root.scrollTo({ left: el.offsetLeft - root.offsetLeft - (root.clientWidth - el.clientWidth) / 2, behavior: 'auto' });
    });
  }, [animations]);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      recenterTimer3.current = setTimeout(handleRecenterIfNeeded3, RECENTER_DEBOUNCE);
    };
    root.addEventListener('scroll', onScroll, { passive: true });
    root.addEventListener('wheel', onScroll, { passive: true });
    root.addEventListener('touchend', onScroll, { passive: true });
    return () => {
      root.removeEventListener('scroll', onScroll);
      root.removeEventListener('wheel', onScroll);
      root.removeEventListener('touchend', onScroll);
      if (recenterTimer3.current) clearTimeout(recenterTimer3.current);
    };
  }, [animations]);
  ...
  useEffect(() => {
    ...
    return () => { obs.disconnect(); document.head.removeChild(styleEl); };
  }, []);
  ...
  return (
    <div id="lm-root" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <section className="lm-hero">
        ...
        <div className="lm-badge"><span className="lm-badge-dot" />Animation Division &middot; UK-Based &middot; AI-First</div>
        <h1 className="lm-h1">Where <span className="lm-highlight">Imagination</span><br />Becomes Reality</h1>
        <p className="lm-hero-desc">La&#x2011;MORKS Technologies is a UK-based Animation powerhouse delivering end-to-end animation production.</p>
        <div className="lm-hero-actions">
          <a href="mailto:info@la-morks.com" className="lm-btn-primary">Start a Creative Project</a>
        </div>
        <div className="lm-mission-box" ref={addRef}>
          ...
          <div className="lm-tag-row">{["2D / 3D Animation","Motion Graphics","VFX","AI Production","Immersive / AR / VR"].map(t => <span key={t} className="lm-tag-violet">{t}</span>)}</div>
        </div>
        {/* <div className="lm-stats">
          {[["50%","AI Production Speedup"],["6","Service Capabilities"],["100%","Client IP Ownership"],["UK","HQ & Leadership"]].map(([n,l]) => (
      ...
      <section className="lm-section" id="about" style={{ background: "#F5F7FF" }}>
        ...
        <h2 className="lm-section-title">Crafting Stories That Move People</h2>
        <p className="lm-section-sub">We combine artistic excellence with AI-powered production to create content that captivates, converts, and endures.</p>
        <div className="lm-mission-box" ref={addRef}>
          <div className="lm-mission-label">Mission Statement</div>
          <h3 className="lm-mission-h3">Bringing Ideas to Life Through the Power of Creative Animation</h3>
          <p className="lm-mission-p">Our mission is to democratise world-class animation for brands of all sizes. By fusing traditional craft with generative AI, we deliver cinematic-quality content at agency speed and startup budgets &#x2014; without compromising on story, style, or substance.</p>
        </div>
        <div className="lm-values-section" ref={addRef}>
          <div className="lm-mission-label">Our Approach</div>
          <div className="lm-values-grid">
            {[
              { num: "01", title: "Story-First Thinking", desc: "Every project starts with narrative. We build compelling stories before we touch a single frame, ensuring the final output resonates with your audience." },
              { num: "02", title: "AI-Accelerated Craft", desc: "Generative AI handles asset creation and variation at scale, freeing our artists to focus on the creative direction, emotion, and detail that machines cannot replicate." },
              { num: "03", title: "Cinematic Quality", desc: "From character design to colour grading, we apply Hollywood-standard techniques to every deliverable &#x2014; regardless of project size or budget." },
              { num: "04", title: "Rapid Iteration", desc: "Real-time rendering and cloud pipelines mean you see progress daily. Fast feedback loops keep creative direction sharp and timelines tight." },
              { num: "05", title: "Your IP, Always", desc: "Full ownership of all deliverables transfers to you. No lock-in, no licensing surprises &#x2014; your creative assets belong to your business." },
            ].map(({ num, title, desc }) => (
      ...
      {/* SERVICES */}
      <section className="lm-section" id="services" style={{ background: "#fff" }}>
      ...
      {/* PORTFOLIO — ANIMATION DELIVERED */}
      <section className="lm-section" id="portfolio" style={{ background: "#F5F7FF" }}>
        <div className="lm-section-tag">Built &amp; Delivered</div>
        <h2 className="lm-section-title">Animation Delivered</h2>
        <p className="lm-section-sub">A selection of animation and creative production work delivered over the last 12 months.</p>
        <div className="lm-portfolio-carousel">
          <div className="lm-portfolio-track" ref={portfolioRef3} aria-label="Animation delivered carousel">
          ...
          <button className="lm-portfolio-prev" aria-label="Previous" onClick={handlePrev3}>&#8249;</button>
          <button className="lm-portfolio-next" aria-label="Next" onClick={handleNext3}>&#8250;</button>
        </div>
      ...
      {/* TECHNOLOGY EDGE */}
      <section className="lm-section" id="tech" style={{ background: "#F5F7FF" }}>
        <div className="lm-section-tag">Technology Edge</div>
        <h2 className="lm-section-title">AI-First. Fast. Cinematic.</h2>
        <div className="lm-tech-content">
          <div className="lm-reveal" ref={addRef}>
            <h3 className="lm-tech-h3">How We Deliver at Speed Without Sacrificing Quality</h3>
            <p className="lm-tech-p">Traditional animation is slow and expensive. We've rebuilt the pipeline around AI-assisted tooling, real-time rendering, and cloud collaboration — compressing weeks into days.</p>
            <div className="lm-tech-list">
              {[
                { icon: "🧠", title: "Intelligent Asset Generation", desc: "AI generates concept art, character variants, and visual assets in seconds — freeing artists to focus on storytelling and emotional direction." },
                { icon: "🔄", title: "Real-Time Rendering", desc: "Unreal Engine and live animation tools enable instant client previews and rapid iteration cycles, eliminating costly revision rounds." },
                { icon: "☁️", title: "Cloud-Based Collaboration", desc: "Distributed pipelines spanning onshore creative leadership and offshore delivery keep production moving 24/7 at scale." },
                { icon: "📈", title: "Trend-Forward Intelligence", desc: "Real-time audience trend analysis ensures every piece of content resonates with today's viewers, not yesterday's." },
              ].map(t => (
      ...
      <section className="lm-section" style={{ background: "#fff" }}>
        ...
        <p className="lm-section-sub">A rare combination of creative artistry, AI-powered tooling, and production discipline — all under one roof.</p>
        <div className="lm-diff-grid" ref={addRef}>
          {[
            { icon: "🌿", title: "GREEN Animation", desc: "Sustainable, fair labour, and energy-efficient practices. Our UK-based leadership appeals to conscious brands and ethical organisations." },
            ...
            { icon: "🔒", title: "Partnership Approach", desc: "Transparent production tracking, daily updates, secured environments, and full asset handover. Your IP remains with you, always." },
            { icon: "🎯", title: "Audience-First Creative", desc: "We research your target audience before a frame is drawn — ensuring every creative decision is grounded in what actually resonates." },
            { icon: "🏆", title: "Enterprise-Grade Precision", desc: "We bring Hollywood-standard rigour, scalability, and reliability to creative workflows — rare in independent animation studios." },
          ].map(d => (
      ...
      <section className="lm-cta-section" id="contact">
        ...
        <p className="lm-cta-p">Whether you need a brand film, product visualisation, social content, or a full animation series &#x2014; we&#x2019;re ready to bring your vision to life.</p>
        ...
        <div className="lm-ideation-box">
          <p className="lm-ideation-title">&#x1F381; Free Creative Brief Session</p>
          <p className="lm-ideation-sub">We&#x2019;ll review your brief, suggest a production approach, and give you an indicative timeline &#x2014; no obligation.</p>
        </div>
      </section>
      ...
      <footer className="lm-footer">
        ...
        <ul className="lm-footer-links">
          {[["#about","About"],["#services","Services"],["/technology","Technology"],["/","Home"]].map(([href,label]) => (
